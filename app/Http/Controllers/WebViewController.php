...
class WebViewController extends Controller
{
    ...
    public function getLbFilter(Request $request)
    {
        ...
        return $this->generatePdfFromLogbook($logbookEntries, $fromDate, $toDate);
    }
    
    private function generatePdfFromLogbook($logbookEntries, $fromDate, $toDate)
    {
    ...
    private function generateLogbookHtml($logbookEntries, $fromDate, $toDate)
    {
        // Generate HTML for the logbook entries table
        $html = '<html>';
        $html .= '<head>';
        $html .= '<style>';
        ...
        $html .= '.title { font-size: 16px; font-weight: bold; margin-bottom: 10px; }';
        $html .= '</style>';
        $html .= '</head>';
        $html .= '<h1>GREENVILLE SUBDIVISION</h1>';

        $html .= '<body>';
        $html .= '<h1>Logbook Records</h1>';
        $html .= '<p class="title">From: ' . $fromDate . ' To: ' . $toDate . '</p>';
        $html .= '<table>';
        $html .= '<tr><th>Visitor</th><th>Homeowner</th><th>Admin</th><th>Personnel</th><th>Destination Person</th><th>Contact Number</th><th>Visit In</th><th>Visit Out</th><th>Visit Members</th></tr>';
    
        foreach ($logbookEntries as $entry) {
            $html .= '<tr>';
            $html .= '<td>' . optional($entry->visitor)->first_name . ' ' . optional($entry->visitor)->last_name . '</td>';
            $html .= '<td>' . optional($entry->homeowner)->first_name . ' ' . optional($entry->homeowner)->last_name . '</td>';
            $html .= '<td>' . optional($entry->admin)->first_name . ' ' . optional($entry->admin)->last_name . '</td>';
            $html .= '<td>' . optional($entry->personnel)->first_name . ' ' . optional($entry->personnel)->last_name . '</td>';
            $html .= '<td>' . $entry->destination_person . '</td>';
            $html .= '<td>' . $entry->contact_number . '</td>';
            $html .= '<td>' . $entry->visit_date_in . ' ' . $entry->visit_time_in . '</td>';
            $html .= '<td>' . ($entry->visit_date_out ? $entry->visit_date_out . ' ' . $entry->visit_time_out : 'Currently In') . '</td>';
    
            $visitMembers = json_decode($entry->visit_members);
    
            if ($visitMembers === null) {
                $html .= '<td>No member found</td>';
            } elseif (is_array($visitMembers)) {
                $html .= '<td>' . implode(", ", $visitMembers) . '</td>';
            } else {
                $html .= '<td>' . $visitMembers . '</td>';
            }
