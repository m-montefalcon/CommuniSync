                ...
                <div class="card">
                    <div class="top-table">
                        <div class="query-form">
                            ...
                            <button class="btn btn-primary filter-button" id="openPdf" data-homeowner-id="{{ $homeowner_id }}">OPEN ALL RECORDS</button>
                        ...
                        <div id="table">
                            <table class="table" id="payment-table">
                                <tbody>
                                    ...
                                    <tr data-homeowner-id="{{ $fetchALlRecord->homeowner_id }}">
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#filterButton").click(function () {
                ...
                // Retrieve homeowner ID from the data-homeowner-id attribute
                var homeownerId = $("#openPdf").data("homeowner-id");

                // Redirect to the specified route with the selected values and homeowner ID as parameters
                window.open("{{ route('api.payments.filter.homeowner') }}?fromMonth=" + fromMonth + "&fromYear=" + fromYear +
                        "&toMonth=" + toMonth + "&toYear=" + toYear + "&homeownerId=" + homeownerId, '_blank');
            });

            document.getElementById('openPdf').addEventListener('click', function() {
                var homeownerId = this.getAttribute('data-homeowner-id');
                var url = "{{ route('api.user.payment.save.records', ['id' => ':homeownerId']) }}";
                url = url.replace(':homeownerId', homeownerId);
